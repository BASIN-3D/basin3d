...
def translate_bbox(a_tuple: tuple) -> tuple:
    """
...
def convert_tuple_to_str(a_tuple: tuple, synthesis_msg=[]) -> Optional[str]:
...
def generator_usgs_measurement_timeseries_tvp_observation(view,
    ...
    monitoring_features_bbox = [translate_bbox(bbox_value) for bbox_value in monitoring_features_bbox]
    monitoring_features_bbox = [convert_tuple_to_str(bbox_coord) for bbox_coord in monitoring_features_bbox]
        ...
        if feature_type in USGSDataSourcePlugin.feature_types or feature_type is None:
            ...
            # no feature_type specified and/or feature_type == POINT
                ...
                if query.monitoring_feature is not None:
                    ...
                    mf_bbox = [translate_bbox(bbox_value) for bbox_value in mf_bbox]
                    ...
                    if mf_bbox:
                        bbox_coords = [convert_tuple_to_str(bbox_tuple) for bbox_tuple in mf_bbox]
